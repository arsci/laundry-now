...
void dryer_algorthum(void)
{
    ...
    int chipselect = 0;
    past_op = 0x00; //This clears the data stores so that it does not get a false read
    past_op = current_op; // This sets the the current operations tp the past operations
    ...
    for(chipselect = 0; chipselect < 8; chipselect++)
    {
        ...
        operation[chipselect] = three_axis(chipselect);
        //if(chipselect == 0) CON3 = operation[0];
        if(operation[chipselect] > 46) //This looks to see if each sensor it set in active mode
        {
    ...
    for(chipselect = 0; chipselect < 8; chipselect++)//out1
    {
        if(testbit(number_sensor,chipselect))//Determines if sensor mode 0-7 nonerror
        {
            ...
            {
                if(testbit(past_op,chipselect))//looks fot the active flag is set in the past_op
                {
                    setbit(CON1,chipselect);
                    clrbit(CON2,chipselect);
                }
            ...
            {
                if(testbit(past_op,chipselect))// looks to see if perivouslty the sensor was in active mode
                {
                    setbit(CON1,chipselect);
                    clrbit(CON2,chipselect);
                }
...
int three_axis(int chipselect){
    ...
    if(chipselect == 0) return (tempT/avg_sensor)/10;
...
void main(void){
    ...
    while(1){
        //for(int chipselect = 0; chipselect < 7; chipselect++){
            dryer_algorthum();
            __delay_ms(50);
            //temp_con2 = three_axis(chipselect);//getVal(X_AXIS,chipselect);
            //if(chipselect == 0) CON3 = temp_con2;
